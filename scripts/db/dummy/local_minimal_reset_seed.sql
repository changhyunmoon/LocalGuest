...
SELECT
  ...
  955000 + n,
  CONCAT(
    -- 게스트별로 도시가 고르게 섞이도록: (n + guestOffset) 기반 인덱스 사용
    ELT(MOD(n + MOD(n - 1, 30), 10) + 1, '서울', '부산', '제주', '강릉', '여수', '경주', '전주', '대구', '광주', '인천'),
    ' ',
  ...
  CASE
    ...
    ELSE CONCAT('희망 지역 ', ELT(MOD(n + MOD(n - 1, 30), 10) + 1, '서울', '부산', '제주', '강릉', '여수', '경주', '전주', '대구', '광주', '인천'))
  END,
