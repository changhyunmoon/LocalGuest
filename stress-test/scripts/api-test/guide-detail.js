...
export default function () {
    ...
    for (let i = 0; i < 2; i++) {
        const res = http.get(`${BASE_URL}/guides/${GUIDE_ID}/detail`)
        check(res, { 'warmup guide detail ok': (r) => r.status === 200 })
        sleep(0.2)
    }

    for (let i = 0; i < 20; i++) {
        const res = http.get(`${BASE_URL}/guides/${GUIDE_ID}/detail`)
        check(res, { 'guide detail ok': (r) => r.status === 200 })
        durations.push(res.timings.duration)
        statuses.push(res.status)
        sleep(0.2)
    }

    logResult('GET /guides/{guideId}/detail', durations, statuses)
}
